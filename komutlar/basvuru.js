...
exports.run = (client, message, args) => {
  ...
  let terfiler = guild.channels.find('name', 'basvuru');
  if (!terfiler) return message.reply('`yetki-başvurusu` kanalını bulamıyorum.');
  let user = message.mentions.users.first();
  if (reason.length < 1) return message.reply('Ne Kazandirirsiniz,Kac Saat Aktif Olacaginiz,Adiniz,Yasiniz ve Kendinizi Etiketleyin.');
